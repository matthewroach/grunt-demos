...
(function( $ ){
  ...
  var methods = {
    ...
    moveDown : function( obj ) {
      ...
      var cTop = nextItem.position(), cOH, total, moveTo;
      if (cTop !== null ) {
        cTop = cTop.top;
        cOH = currentItem.outerHeight();
        total = parseInt(cTop, 10) + parseInt(cOH, 10);

        if (total >= obj.dropH) {
          moveTo = parseInt(obj.dropH, 10) - parseInt(total, 10);
    ...
    moveUp : function( obj ) {
      ...
      var cTop = prevItem.position(), cOH, total, moveTo;
      if (cTop !== null ) {
        cTop = cTop.top;
        cOH = currentItem.outerHeight();
        total = parseInt(cTop, 10) + parseInt(cOH, 10);
    ...
    selectItem : function ( obj, item ) {
      var selected;
      if (item === undefined) {
        selected = $(obj.list+' li.aw-highlighted');
      } else {
        selected = $(item);
      }
